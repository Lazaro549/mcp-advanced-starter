...
describe("NotesStore", () => {
  ...
  describe("search", () => {
    ...
    it("handles concurrent note creation without losing notes", async () => {
      const notes = await Promise.all(
        Array.from({ length: 100 }, (_, index) =>
          Promise.resolve(store.create({ title: `Concurrent ${index}`, body: "body", tags: [], priority: "low" }))
        )
      );

      expect(new Set(notes.map((note) => note.id)).size).toBe(100);
      expect(store.all()).toHaveLength(103);
    });
  });

  describe("importFromRoot", () => {
    it("rejects non-file roots before touching the filesystem", async () => {
      await expect(store.importFromRoot("https://example.com/notes")).rejects.toThrow("file://");
    });
  });
});
